...
async function getOAuthClient(): Promise<NodeOAuthClient> {
  ...
  _oauthClient = new NodeOAuthClient({
    clientMetadata: {
      ...
      scope: 'atproto repo:app.bsky.feed.post repo:app.bsky.feed.like repo:app.bsky.feed.repost blob:*/* account:email',
...
authRouter.get('/oauth/login', async (c) => {
  ...
  try {
    ...
    const url = await client.authorize(handle, { scope: 'atproto repo:app.bsky.feed.post repo:app.bsky.feed.like repo:app.bsky.feed.repost blob:*/* account:email' });
